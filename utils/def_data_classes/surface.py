...
from utils.def_data_classes.line import AllLines, DefLine, get_line_id_using_coordinates
...
class DefSurface:
    ...
    def __post_init__(self):
        if self.boundary_lines_id:
            self.lines_def_surface.extend(
                get_lines_def_surface(boundary_lines_id=self.boundary_lines_id)
            )
...
class DefLoadDistribution:
    ...
    def __post_init__(self):
        if self.boundary_lines_no:
            self.lines_def_surface.extend(
                get_lines_def_surface(boundary_lines_id=self.boundary_lines_no)
            )
...
def get_boundary_lines_no(corners_of_the_surface, all_lines, all_nodes) -> List[int]:
    ...
    for (
        _index,
        _def_node,
    ) in enumerate(corners_of_the_surface):
...
class AllSurfaces:
    ...
    all_def_surfaces: List[Union[DefSurface, DefLoadDistribution]] = field(
        default_factory=list
    )

    def create_surface_by_no(
        self, boundary_lines_no: List[int], thickness: int, id: Optional[int] = None
    ) -> DefSurface:
        new_id = get_new_max_id(all_ids=self.all_ids, id=id)
        self.all_ids.append(new_id)
        new_def_surface = DefSurface(
            boundary_lines_id=boundary_lines_no, thickness=thickness, id=new_id
        )
        Surface(
    ...
    def create_load_distribution_by_nodes(
        self,
        corners_of_the_surface: List[DefNode],
        all_lines: AllLines,
        id: Optional[int] = None,
        ...
        new_load_distribution = DefLoadDistribution(
            id=new_id,
    ...
    def copy_and_offset(
        self,
        def_surface: Union[DefSurface, DefLoadDistribution],
        all_lines: AllLines,
        all_nodes: AllNodes,
        ...
        id: Optional[int] = None,
    ) -> Union[DefSurface, DefLoadDistribution]:
        ...
        if def_surface.__class__.__name__ == "DefSurface":
            new_def_opening = self.create_surface_by_nodes(
                corners_of_the_surface=corners_of_the_surface,
